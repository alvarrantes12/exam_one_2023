...
class CapturedPokemonsController < ApplicationController
  ...
  def create
    ...
    if @captured_pokemon.save
      render 'api/captured_pokemons/show', status: :created, location: @captured_pokemon
    else
  ...
  def update
      if @captured_pokemon.update(captured_pokemon_params)
        render 'api/captured_pokemons/show', status: :ok, location: @captured_pokemon 
      else
        render json: @captured_pokemon.errors, status: :unprocessable_entity 
      end
  end
  ...
  def destroy
    ...
    render 'api/captured_pokemons/index', status: :ok
  end
