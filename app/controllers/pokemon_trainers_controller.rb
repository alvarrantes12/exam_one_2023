class PokemonTrainersController < ApplicationController
  ...
  def create
    ...
    if @pokemon_trainer.save
      redirect_to pokemon_trainer_url(@pokemon_trainer), notice: t('application.created_model', model: t('activerecord.modules.pokemon_trainer.one'))
    else
  ...
  def update
    if @pokemon_trainer.update(pokemon_trainer_params)
      redirect_to pokemon_trainer_url(@pokemon_trainer), notice: t('application.edited_model', model: t('activerecord.modules.pokemon_trainer.one'))
    else
  ...
  def destroy
   ...
   redirect_to pokemon_trainers_url, notice: t('application.deleted_model', model: t('activerecord.modules.pokemon_trainer.one'))
  end
