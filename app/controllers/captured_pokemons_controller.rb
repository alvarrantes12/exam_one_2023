class CapturedPokemonsController < ApplicationController
  ...
  def index
    ...
    render json: @captured_pokemons.to_json(only: [:id], include: { pokemon: { only: [:name] }, pokemon_trainer: { only: [:first_name, :last_name] } })
  end

  def show
    @captured_pokemon = CapturedPokemon.find(params[:id])
    render json: @captured_pokemon.to_json(include: { pokemon: { only: [:name] }, pokemon_trainer: { only: [:first_name, :last_name] } })
  end
