require 'rails_helper'

RSpec.describe PokemonTrainer, type: :model do
  subject{
    described_class.new(first_name: "Test",
                        last_name: "Test L",
                        id_number: "12345678")
  }

  it "is valid with valid attributes" do
    expect(subject).to be_valid
  end

  it "is not valid without first_name" do
    subject.first_name = nil
    expect(subject).to_not be_valid
  end

  it "is not valid with first_name with more than 10 letters"do
    subject.first_name = "kendaaaaaaaalllllll"
    expect(subject).to_not be_valid
  end

  it "is not valid without last_name" do
    subject.last_name = nil
    expect(subject).to_not be_valid
  end

  it "is not valid with last_name with more than 10 letters"do
    subject.last_name = "Conejoooooooooooo"
    expect(subject).to_not be_valid
  end

  it "is not valid without id_number" do
    subject.id_number = nil
    expect(subject).to_not be_valid
  end

  it "is not valid with id_number with less than 8 numbers"do
    subject.id_number = "12345"
    expect(subject).to_not be_valid
  end

  it "is not valid with id_number with more than 10 numbers"do
    subject.id_number = "1234567890123"
    expect(subject).to_not be_valid
  end

  it "is not valid with a duplicated id_number" do
    existing_pokemon_trainer = described_class.create(first_name: "Jose", last_name: "Picado", id_number: "728354719")
    subject.id_number = existing_pokemon_trainer.id_number
    expect(subject).to_not be_valid
  end
end
